...
DWORD  CALLBACK  remote_main(BYTE* image_base)
{
   #pragma  region Initialize module image
	...
	for (size_t i = 0; import_directory_entries[i].FirstThunk != 0; i++)
	{
		...
		//  It is  safe to call 'LoadLibrary'  here because  the IAT  entry  for it  was  copied  from  the parent proces  and KERNEL32.dll  is always loaded  at the  same address
	...
	//  Run  main loop 
	blink_parser::Application().Run(blink_handle, blink_environment, blink_working_directory);
...
int main(int argc, char* argv[])
{
	...
	{
		std::cout << "Failed to allocate and write  'LoadLibrary' argument  application!" << std::endl;
		return GetLastError();
	}

	// Execute 'LoadLibrary'  in target  application 
	const Scoped_Handle load_thread = CreateRemoteThread(remote_process, nullptr, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(&LoadLibraryW), load_param, 
		0, nullptr);
	...
	//  Make the entire image  writeable so the copy operation below can succeed
